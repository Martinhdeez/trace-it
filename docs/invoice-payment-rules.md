...
The seventeen rules ship with hand-written code, `processes/rules-v3/r01-...py` to `r17-...py` (one file per rule, in the order of the JSON); `make activate` (`python -m app.cli load ... --activate`) puts them into the process without any model. The compiler (`make compile`) can regenerate the code from the texts with a blind tester and a coder (ADR 0004), and `make eval-compiler` compares what it produces with the hand-written code on the golden instances of batch 1. Rule numbers here are the file numbers, R01 to R17. Note: three texts in the JSON still refer to other rules by an older 0-based number: "(R01)" and "(R03)" inside R03 mean R02 and R04, and "R04" inside R13 means R05.
...
| R03 | `r03-master-iban.py` | If there are rows in `suppliers` with that `nif`, and all of them have the same `id` and normalised `iban`, then that `iban` equals the normalised `iban` of the invoice, or differs from it in more than 4 characters. If there are no rows, it does not fire (R01). If the rows disagree with each other, it does not fire (R03). A difference of at most 4 characters is a misreading rather than another account: R17 escalates it. | R | NO_PAGAR | 1 | FA-7311, FA-4290, FA-5633, FA-9104, FA-5044_mensajería2, F26-8812, 2026-07-08_P010, F26-9007 |
...
| R17 | `r17-iban-near-miss.py` | The normalised `iban` of the invoice differs from the one account the master approves for that `nif` in at least 1 and at most 4 characters. Reason: both accounts and how many characters apart they are. | P | ESCALAR | 1, 6 | none in batch 1: every real mismatch is 15 to 20 characters away |
