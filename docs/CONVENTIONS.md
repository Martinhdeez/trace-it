...
- **Engine decision**: a `decisions` row with `author = "engine"`. **Final decision**: an instance's latest row, whoever wrote it. **Exported decision**: the engine's latest decision; a person's decision is exported only when the engine never decided that instance (ADR 0016). A `PENDING` instance exports the process's escalation type with reason `NO_FINDING` until the engine decides it.
- **Escalation type**: the process's highest-priority `requires_human` type. The engine sends there any instance missing a required symbol (`MISSING_DATA: ...`), whose rule code failed (`RULE_ERROR ...`), whose rule is `blocked` for missing data (`RULE_NEEDS_DATA ...`) or a failed compile (`RULE_COMPILE_FAILED ...`) or whose fired types tie on priority (`RULE_CONFLICT ...`). There is no REVIEW state.
...
- **Decision review**: optional advice linked to an engine decision, with a recommendation, reasoning and evidence references. A disagreement requires human approval; the advice never changes rule findings or the engine decision. `review_pending` is separate from instance status and outcome. See [ADR 0021](adr/detail/0021-optional-decision-review.md).
- **Reviewed export**: when the latest engine decision has a review record, export waits for pending approval and uses a later human resolution when present. Decisions without review retain the engine-first export described above.
