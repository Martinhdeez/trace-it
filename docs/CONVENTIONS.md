...
- **Escalation type**: the process's highest-priority `requires_human` type. The engine sends there any instance whose rule code failed (`RULE_ERROR ...`) or whose fired types tie on priority (`RULE_CONFLICT ...`). There is no REVIEW state.
