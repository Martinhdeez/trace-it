...
> **Fuente de verdad:** `procesos/pago-facturas.json` (lo que se carga con `make setup`). Este documento explica de dónde sale cada regla.
>
...
>
> Ya decidido: R16 no es una regla (será un aviso en la traza); recargo financiero y fechas imposibles → NO_PAGAR; hoja `pendiente_revisar` → sin regla; pedido duplicado (R15) → ESCALAR, con las instancias identificadas por `_instancia`.
...
- **Firma:** `evaluar(instancia, fuentes, otras) -> {"salta": bool, "motivo": str}`. La decisión la fija la regla en su definición (P18). Función pura: sin reloj, red ni disco.
- **Importes:** se comparan en céntimos enteros: `round(x * 100)`. "Coincide (±0,01)" significa `abs(centimos(a) - centimos(b)) <= 1`.
- **Normalización de claves** (antes de comparar, en la regla): `nif` → mayúsculas, sin espacios, guiones ni puntos. `iban` → mayúsculas, sin espacios. `pedido` → mayúsculas, sin espacios. Textos de maestros (`razon_social`) → `strip()`.
- **Símbolo ausente** (`None`): una regla que lo necesita **no salta** (devuelve `salta=False`), salvo R00, que es la única que castiga la ausencia. Así un campo ausente produce un solo motivo y no una cascada.
...
| `nif_emisor` | str\|None | NIF/CIF del proveedor que emite. Nunca el del cliente (`A58231074`, Banco Miralmar). |
| `razon_social_emisor` | str\|None | Nombre del emisor tal como aparece. Solo traza; ninguna regla decide con él. |
| `iban` | str\|None | IBAN de abono impreso en la factura. |
| `numero_factura` | str\|None | Número de factura tal cual (`FA-5077`, `2026/0811-B`, `F26-0233`). |
| `fecha` | str\|None | Fecha de emisión como `AAAA-MM-DD` **con los números impresos, sin corregir**. `31/02/2026` → `"2026-02-31"`. Acepta `DD/MM/AAAA` y `6 de abril de 2026`. |
| `pedido` | str\|None | Referencia de pedido (`PO-AAAA-NNNN`). |
| `base` | decimal\|None | Base imponible / Subtotal. |
| `tipo_iva` | decimal\|None | Porcentaje de IVA **impreso** (`21` en `IVA (21%)`). |
| `cuota_iva` | decimal\|None | Cuota de IVA impresa. |
| `total` | decimal\|None | Total impreso (`TOTAL`, `IMPORTE TOTAL`, `TOTAL A PAGAR`, `Total factura`). No `Subtotal` ni `Suma y sigue`. |
| `texto_libre` | str | Texto que no es cabecera, línea de concepto ni totales (notas, avisos, condiciones). Siempre se guarda. Ninguna regla lo lee: alimenta el aviso de instrucciones incrustadas en la traza (ver 3.1). |
...
|---|---|---|
| `proveedores` | `id`, `razon_social`, `nif`, `iban` | Hoja `Proveedores` (ID, Razon Social, NIF, IBAN). **Se cargan todas las filas, también duplicadas** (P007 aparece en filas 8 y 13, idénticas). `razon_social` con `strip()` (`"Ofimática Cieza S.L.  "`). |
| `pedidos` | `pedido`, `proveedor_id`, `nif`, `importe_total`, `estado`, `fecha_pedido` | Hoja `Pedidos_2026` (516 filas, todas `ABIERTO`). `nif` puede venir vacío (`None`): PO-2026-0538 a 0557. |
| `erp` | `asiento_id`, `fecha`, `proveedor_id`, `nif`, `pedido`, `importe`, `estado` | Foto local de la API (`<id>`, `<fecha>`, `<proveedor>`, `<nif>`, `<pedido>`, `<importe>`, `<estado>`), ya convertida: fecha ISO, importe decimal, texto decodificado de ISO-8859-1. `estado` ∈ {`PENDIENTE`, `PAGADA`}. |
| `parametros` | `fecha_corte` | Una fila. Fecha de referencia para "no futura" (la regla no puede leer el reloj). Queda en la traza. |

`otras`: lista de instancias (mismos símbolos + la clave `_instancia`, su nombre de fichero) del mismo proceso **de todos los lotes ya ingeridos**, sin la propia.
...
|---|---|---|---|---|---|
| R00 | Los símbolos `nif_emisor`, `iban`, `pedido`, `fecha`, `base`, `tipo_iva`, `cuota_iva` y `total` son todos distintos de `None`. Motivo: lista de los que faltan. | R | ESCALAR | 6 (duda razonable) | ninguno en texto |
| R01 | Existe al menos una fila en `proveedores` cuyo `nif` normalizado es igual a `nif_emisor` normalizado. | R | NO_PAGAR ⚠ | 1 | FA-2508_consultoría (B87654321), factura_4485, factura_7265 (B41908877) |
| R02 | Si hay filas en `proveedores` con ese `nif`, y todas tienen el mismo `id` e `iban` normalizado, entonces ese `iban` es igual a `iban` normalizado de la factura. Si no hay filas, no salta (R01). Si las filas discrepan entre sí, no salta (R03). | R | NO_PAGAR ⚠ | 1 | FA-7311, FA-4290, FA-5633, FA-9104, FA-5044_mensajería2, F26-8812, 2026-07-08_P010, F26-9007; escaneo reimpresion_0712 |
| R03 | Hay dos o más filas en `proveedores` con el mismo `nif` normalizado que `nif_emisor` y distinto `id` o distinto `iban` normalizado (filas idénticas repetidas NO cuentan). | P | ESCALAR | 6 | ninguno (P007 está duplicado pero idéntico) |
| R04 | Existe una fila en `pedidos` cuyo `pedido` es igual a `pedido` de la factura. | R | NO_PAGAR | 2 | FA-2508 (PO-2026-9999), factura_4485 (PO-2026-0806), factura_7265 (PO-2026-0706) |
| R05 | Con la fila de `pedidos` de ese `pedido`: si `pedidos.nif` no está vacío, es igual a `nif_emisor`; si está vacío, `pedidos.proveedor_id` es igual al `id` de la fila de `proveedores` con `nif = nif_emisor` (si esa fila no existe, no salta). | R | NO_PAGAR | 2 | 2026-07-08_P010 (PO-1206), F26-9007 (PO-1205) |
| R06 | Con la fila de `pedidos` de ese `pedido`: `total` coincide con `pedidos.importe_total` (±0,01). Se compara el **total** de la factura, no la base. | R | NO_PAGAR | 2 | factura_1936, factura_2018, factura_3184, factura_8801, FA-5077 (12.874,40 vs 12.847,40); también las 6 de IVA y las 2 de recargo |
| R07 | `cuota_iva` coincide (±0,01) con `round(base * tipo_iva / 100, 2)`, usando el `tipo_iva` impreso. | R | NO_PAGAR | 3 | F26-5240, F26-6702, F26-6964, F26-9012, FA-5590 (cuota al 16 %), F26-8801 (cuota al 10 %) |
| R08 | `tipo_iva` es distinto de 21. | P | ESCALAR ⚠ | 3 + `notas_alberto` fila 4 ("IVA reducido (aplica??)") | ninguno (las 471 imprimen 21 %) |
| R09 | `total` coincide (±0,01) con `base + cuota_iva`, usando los valores impresos. El texto de la factura no cambia el cálculo. | R | NO_PAGAR [DECIDIDO] | 3 | 2026-0811-B_catering, 2026-14500-C_informática ("recargo … abonarse el total impreso") |
| R10 | `fecha` es una fecha real del calendario (`AAAA-MM-DD` válido: mes 1-12 y día existente en ese mes y año). | R | NO_PAGAR [DECIDIDO] | 4 | 2026-03-19_P008 (31/02), FA-1123 (30/02), FA-2967 (31/02) |
| R11 | Si `fecha` es válida: `fecha` es posterior a `parametros.fecha_corte`. | P | NO_PAGAR [DECIDIDO] | 4 | ninguno (fecha máxima del lote: julio 2026) |
| R12 | Existe una fila en `erp` cuyo `pedido` es igual a `pedido`. Solo se evalúa si R04 no salta. | R | NO_PAGAR | 5 + `notas_alberto` ("NUNCA pagar sin cruzar con el ERP") + DISCREPANCIA_EXCEL_ERP [DECIDIDO] | ninguno (Excel y ERP tienen los mismos 516 pedidos) |
| R13 | Con las filas de `pedidos` y `erp` de ese `pedido`: `erp.importe` coincide con `pedidos.importe_total` (±0,01) **y** `erp.proveedor_id` es igual a `pedidos.proveedor_id` **y**, si ambos `nif` no están vacíos, son iguales. Motivo: campos que difieren. | R | NO_PAGAR | DISCREPANCIA_EXCEL_ERP [DECIDIDO] | ninguno en texto; afecta a PO-2026-0538…0557 (17 de 20 con `proveedor_id` distinto, NIF vacío en ambos) si aparecen en escaneos o lote 2 |
| R14 | Con la fila de `erp` de ese `pedido`: `erp.estado` es `PAGADA` (en realidad: distinto de `PENDIENTE`). | P | NO_PAGAR | 5 | 2026-03-28_P002, 2026-04-08_P007, 2026-05-28_P003, 2026-06-04_P006, 2026-17547, FA-1016, FA-2116, factura_4619, factura_5911 |
| R15 | Existe en `otras` al menos una instancia con el mismo `pedido` normalizado. Cada instancia de `otras` se identifica por su clave `_instancia` (su nombre de fichero). Motivo: las `_instancia` con ese pedido. | P | ESCALAR [DECIDIDO] | 5 | factura_41082 (F26-0233) + 2026-0233-A_catering (PO-2026-0492) |
...
- **Espacios finales en `Proveedores`:** `strip()` al cargar; ninguna regla usa `razon_social`.
...
- **Validadores mod-97 del IBAN y letra del NIF:** son de extracción (P20), no de negocio. Si fallan, la instancia va a `REVISION`.
...
**Recomendación: NO_PAGAR** para ambas: la norma 4 dice "La fecha debe ser válida y no futura" y la fecha no se corrige (reglas-sistema §3.1). **Confianza: media-alta.** R11 no afecta al lote 1; `fecha_corte` se fija al día de proceso y se documenta.
...
### 3.7 Pedido duplicado (R15): ¿alcance de `otras`?
La decisión (ESCALAR las dos) ya está tomada. Queda abierto si `otras` incluye facturas de lotes anteriores. **Recomendación: sí**, todos los lotes del proceso: si el lote 2 trae otra factura de un pedido ya facturado, las dos pasan a ESCALAR y la del lote 1 genera un aviso de auditoría (P14), sin cambiar el pasado. **Confianza: media.**

### 3.8 ERP: pedido ausente o distinto del Excel (R12, R13)
NO_PAGAR ya está decidido. Solo recuerdo que afecta a PO-2026-0538…0557 (NIF vacío en Excel y ERP; `proveedor_id` distinto en 17 de 20). Ninguna factura de texto del lote 1 los cita; pueden aparecer en escaneos o en el lote 2.
...
| IVA "bien calculado": ¿con el 21 % fijo o con el tipo impreso? | R07 usa el `tipo_iva` impreso; R08 aparte para el tipo ≠ 21. Así una cuota al 10 % con etiqueta 21 % sale NO_PAGAR (R07) y no depende de R08. |
| Redondeo de la cuota | `round(base * tipo_iva / 100, 2)` y luego tolerancia de 1 céntimo. |
| Pedido de otro proveedor cuando `pedidos.nif` está vacío | R05 cae a `proveedor_id` vía la fila del maestro. |
...
| Fecha imposible: ¿el extractor la "arregla"? | El símbolo `fecha` guarda los números impresos sin validar (`"2026-02-31"`); la validación es de R10. El extractor no debe devolver `None` por una fecha imposible (eso la convertiría en R00/ESCALAR). |
| "Futura": ¿respecto a qué? | `parametros.fecha_corte`; la función no lee el reloj (P18). |
| Duplicado: ¿clave `numero_factura` o `pedido`? | `pedido`. `FA-8801` aparece en factura_8801 (Guadaira) y 2026-05-28_P005 (Hermanos Pico): mismo número, distinto proveedor y pedido; no es duplicado. `otras` no incluye la propia instancia; cada una se identifica por `_instancia` (nombre de fichero). |
| Comparación de claves (espacios en IBAN, NIF con guion) | Normalización fija en las convenciones. |
| `erp.estado` con valores inesperados | R14 salta con cualquier valor distinto de `PENDIENTE`. |
| Texto libre como entrada de decisión | No hay regla sobre él (3.1). Ninguna otra regla puede leer `texto_libre`: debe decirse así en el prompt de compilación. |
| NIF del cliente tomado como emisor | Excluido en la definición de `nif_emisor` (`A58231074`). |
