...
| | `active` | Compiled, tests passed: already enforced. The invoice use case has no share limit (`auto_activate_max_change: 1.0`, ADR 0004), so a valid rule activates whatever it changes; its impact is recorded as audit findings | Check, not a gate: read its `interpretation` (`GET /rules/{id}`) and what it changes (`GET $API/processes/1/findings`). A wrong reading: retire it (`POST /rules/{id}/retire`) and use 6d |
...
| | `blocked` with `report.error` | The compile failed (model down, tokens out, malformed output). Enforced as ESCALAR for every instance, reason `RULE_COMPILE_FAILED` | `POST $API/rules/{id}/compile` once the model answers; still failing: 6d |
| | `blocked` with `report.needs_data` | The rule needs a symbol or source the process lacks | Enforced as ESCALAR for every instance it runs on. Decide before step 7: provide the data (new source / symbol) and recompile, or retire it (`POST /rules/{id}/retire`) if the norm does not really need it |
...
| 8b | `curl -s "$API/processes/1/queue" \| python3 -c 'import json,sys; from collections import Counter; print(Counter(i["reason"][:40] for i in json.load(sys.stdin)))'` | ESCALAR reasons: `MISSING_DATA` (scans), the v4 rule, R16 duplicates, IBAN | Any `RULE_ERROR`, `RULE_NEEDS_DATA` or `RULE_COMPILE_FAILED`: a rule failed or is blocked; fix it (6c) and step 9 |
