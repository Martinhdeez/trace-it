...
| Export | `GET /processes/{id}/export?trace=` | `outcomes.jsonl`; `trace=true` adds `trace_url` per line (`<TRACE_CONSOLE_BASE_URL>/processes/{id}/review?i=<instance>`, the same screen the CLI's `export --trace` writes); pending instances use the process escalation outcome and `NO_FINDING`, while 409 remains for reviewer-requested approval or another missing outcome. One batch only: `make export-batch` (`docs/runbook-batch2.md`) |
