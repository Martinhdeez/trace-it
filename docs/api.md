...
| Export | `GET /processes/{id}/export` | `outcomes.jsonl`; pending instances use the process escalation outcome and `NO_FINDING`, while 409 remains for reviewer-requested approval or another missing outcome. One batch only: `make export-batch` (`docs/runbook-batch2.md`) |
