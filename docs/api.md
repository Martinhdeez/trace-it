...
| Treasury | `POST /processes/{id}/treasury/preview` | read-only weekly payment scenario with explicit date, budget and horizon; [planning contract](treasury-planning.md) |
| Assistant | `GET /instances/{id}/suggestion` | decision, `why` (≤ 2 lines), `reasoning` (≤ 2 sentences), `options` (each a one-line `consequence` and the English `rule` that justifies it), `proposed_rule`, or `no_rule_reason` when a person must always decide such cases (then `proposed_rule` is null). On an engine escalation (MISSING_DATA, SOURCE_UNAVAILABLE, RULE_CONFLICT...) `no_rule_reason` is always set and `options` adds the human type ("Mantener escalado y pedir el dato"), which may be the proposed one. All Spanish text is plain: no field names, rule codes or references. 409 if not escalated, 502 if the model failed |
...
| Export | `GET /processes/{id}/export?trace=&full=` | `outcomes.jsonl`; `trace=true` adds `trace_url` per line (`<TRACE_CONSOLE_BASE_URL>/processes/{id}/review?i=<instance>`, the same screen the CLI's `export --trace` writes); pending instances use the process escalation outcome and `NO_FINDING`, while 409 remains for reviewer-requested approval or another missing outcome. `full=true` (CLI `export --full`) writes the whole trace inline instead, always after `file_id` and `result`: `reason_code`, `reason`, `decided_by` (`engine`/`person`, and a person's later resolution is then the exported one), `decided_at`, `process_version`, `rules_hash`, `rules_fired`, `evidence`, `sources_read`, `trace_id` and `trace_url` last, each left out when it has no data. One batch only: `make export-batch` (`docs/runbook-batch2.md`) |
...
| Usage explorer | `GET /processes/{id}/metrics/breakdown?plane=&module=&since=&until=&through_id=&offset=&limit=` | `UsageBreakdown`: overview groups by plane; selecting a plane groups by module; selecting a module groups by model/provider and includes paginated operations. `series` supplies time buckets; `flow` supplies additive plane/module/provider/model paths for the Sankey. `totals` is null at the overview level. Reuse the returned `until` and `through_id` for stable pagination. See [usage accounting](observability-dashboards.md#usage-explorer) |
