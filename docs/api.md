...
| Resolve | `POST /instances/{id}/resolve` `{decision, reason, proposal_id?}` | adds a decision; the engine's stays. With `proposal_id`, the resolution event records it and settles that proposal |
