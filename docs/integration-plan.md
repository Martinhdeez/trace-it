...
| 1.9 | Metrics / health: **three traceability dashboards** | One dashboard per plane: **Ingestion** (data and OCR), **Agents** (norm → code) and **Execution** (published rules on cases). Each shows its own activity, quality, tokens and cost from `/processes/{id}/metrics/{plane}`, drills down to the spans, and never mixes planes in one total. Spec: [`docs/observability-dashboards.md`](observability-dashboards.md). They replace the "—" placeholders. `/health/planes` goes on each dashboard header and as a sidebar dot (#20) | `FE:routes/Process.tsx`, `FE:components/shell/Sidebar.tsx` | 3 |
