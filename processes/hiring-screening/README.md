...
| `criminal_records_erp.py` | Synthetic legacy criminal records used by the published demo version | Connector discovery and normal pre-run synchronization; see `CRIMINAL_RECORDS_ERP.md` |
...
The deployed demo adds the reviewed criminal-records connector after this original
discovery experiment. A normalized full-name match is a direct `REJECT`; Ana Molina in
`cv-001.pdf` matches `CR-00001`. The deployment seed caches 41 CV readings and the registry
snapshot. It leaves `cv-002.pdf`, `cv-021.pdf` and `cv-024.pdf` for live uploads.
