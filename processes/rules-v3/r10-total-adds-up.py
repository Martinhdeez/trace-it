...
def evaluate(instance, sources, others):
    ...
    for symbol in ["base", "vat_amount", "total"]:
        if empty(instance.get(symbol)):
            return passes()
    total = Decimal(str(instance.get("base"))) + Decimal(str(instance.get("vat_amount")))
    if not matches(instance.get("total"), total):
        return fires("Total " + text(instance.get("total")) + " != base + VAT " + str(total))
    return passes()
