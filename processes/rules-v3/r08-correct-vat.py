...
def evaluate(instance, sources, others):
    ...
    if empty(instance.get("vat_amount")):
        return passes()
    base = Decimal(str(instance.get("base")))
    rate = Decimal(str(instance.get("vat_rate")))
    expected = (base * rate / Decimal("100")).quantize(Decimal("0.01"), rounding=ROUND_HALF_UP)
    if not matches(instance.get("vat_amount"), expected):
        return fires("VAT " + text(instance.get("vat_amount")) + " != " + str(expected))
    return passes()
