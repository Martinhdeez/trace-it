...
| `agents` | `{role: settings}`, roles `compiler`, `tester`, `assistant`, `normalizer`. Settings: `model` (`provider:model` or `helmcode:<id>`; default `TRACE_<ROLE>_MODEL`), `fallback_models` (models tried in order when the one before fails at the provider: 5xx, 429, timeout, connection; ADR 0019), `timeout_seconds` (per request to the provider), `retries` (answers the output validators may send back; each one is a whole extra call), `instructions` (domain guidance appended to the platform prompt in `backend/app/features/agents/prompts/`), `model_settings` (e.g. `{"temperature": 0}`), `limits` (compiler: `max_attempts`, `auto_activate_max_change`, `1.0` = a valid rule always activates; tester: `min_tests`, `max_reviews`; any role: `http_retries`, how often the SDK repeats a failed request before the next model, default 2, each repeat waiting a whole timeout), `failed_check_decision` (normalizer only: the decision of a check whose failure the norm does not name, e.g. "pay only if X"; a decision type of the process, never the default; without it the normalizer falls back to the norm's tie-breaker, then the most conservative type that requires a human), `examples` (`[{text, type, code}]`, `code` a path relative to this file; the compiler sees the examples of the other rules, never the one being compiled) |
