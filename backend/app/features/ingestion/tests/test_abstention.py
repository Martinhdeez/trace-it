...
def test_single_reader_proposal_is_preserved_but_not_used_as_verified_value(settings):
    class OCR(NoOCR):
    ...
    assert field["proposed_value"] == "B96233419"
    assert field["verification"] == "unverified"
    assert field["selected_by"] is None
    assert field["proposed_by"] == "primary"
    assert field["agreeing_readers"] == []
...
def test_visual_disagreement_keeps_alternatives_and_returns_a_reading(settings):
    ...
    assert result.fields["purchase_order_ref"].proposed_value == "PO-2026-0703"
